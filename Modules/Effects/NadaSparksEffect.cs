    ...
    internal sealed class NadaSparksEffect : MonoBehaviour, INadaItemDataReceiver
    {
        ...
        private void RebuildSparkEmitters()
        {
            ...
            foreach (Transform child in transform)
            {
                ...
                if (!_baseEmitterPosition.ContainsKey(id))
                    _baseEmitterPosition[id] = child.localPosition;
            }
        }
        ...
        private void ApplyScale(float scale)
        {
            ...
            float sparksMult = ClampScale(scale);
            ...
            foreach (var ps in _systems)
            {
                ...
                try
                {
                    ...
                    if (_baseStartSize.TryGetValue(id, out var baseStartSize))
                        main.startSize = ScaleMinMaxCurve(baseStartSize, sparksMult);

                    if (_baseStartLifetime.TryGetValue(id, out var baseStartLifetime))
                        main.startLifetime = ScaleMinMaxCurve(baseStartLifetime, sparksMult);

                    if (_baseSimulationSpeed.TryGetValue(id, out var baseSimulationSpeed))
                    {
                        float simulationSpeedMult = sparksMult < 1f ? sparksMult : 1f;
                ...
                try
                {
                    ...
                    if (wasEnabled && _baseSizeOverLifetime.TryGetValue(id, out var baseSize))
                        sizeOverLifetime.size = ScaleMinMaxCurve(baseSize, sparksMult);
                }
                catch { }
            }
        }
        ...
        private void ApplyLength(float length, float position)
        {
            ...
            int activeCount = Mathf.Clamp(
                Mathf.RoundToInt(Mathf.Lerp(1f, _sparkEmitters.Count, t)),
                1,
                _sparkEmitters.Count);

            float spread = Mathf.Lerp(0f, SparksMaxSpread, t);
            ...
            for (int i = 0; i < _sparkEmitters.Count; i++)
            {
                ...
                int id = sparkTransform.GetInstanceID();

                Vector3 basePosition =
                    _baseEmitterPosition.TryGetValue(id, out var cached)
                        ? cached
                        : Vector3.zero;
            ...
            foreach (var ps in _systems)
            {
                ...
                try
                {
                    ...
                    emission.rateOverTime =
                        ScaleMinMaxCurve(baseRateOverTime, emissionMult);

                    emission.rateOverDistance =
                        ScaleMinMaxCurve(baseline.RateOverDistance, emissionMult);
                }
                catch { }
            }
        }
