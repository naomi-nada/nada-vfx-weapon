// File: State/VfxStateKeys.cs
namespace NADA.VFX.Core.State
{
    internal static class VfxStateKeys
    {
        internal const string InnerHueShift = "nada.vfx.inner.hue";
        internal const string OuterHueShift = "nada.vfx.outer.hue";
        internal const string FlareHueShift = "nada.vfx.flare.hue";

        internal const string InnerEnergy = "nada.vfx.inner.energy";
        internal const string OuterEnergy = "nada.vfx.outer.energy";

        internal const string InnerChaos = "nada.vfx.inner.chaos";
        internal const string OuterChaos = "nada.vfx.outer.chaos";

        internal const string MirageEnabled = "nada.vfx.mirage.enabled";
        internal const string MirageHueShift = "nada.vfx.mirage.hue";
        internal const string MirageScale = "nada.vfx.mirage.scale";

        internal const string SparksEnabled = "nada.vfx.sparks.enabled";
        internal const string SparksHueShift = "nada.vfx.sparks.hue";
        internal const string SparksEnergy = "nada.vfx.sparks.energy";
        
        internal const string OrbitalsOrbsEnabled = "nada.vfx.orbitals.orbs.enabled";
        internal const string OrbitalsOrbsCount = "nada.vfx.orbitals.orbs.count";
        internal const string OrbitalsOrbsScale = "nada.vfx.orbitals.orbs.scale";
        internal const string OrbitalsOrbsHueShift = "nada.vfx.orbitals.orbs.hue";
        internal const string OrbitalsOrbsSpacing = "nada.vfx.orbitals.orbs.spacing";
        internal const string OrbitalsOrbsRadius = "nada.vfx.orbitals.orbs.radius";

        internal const string OrbitalsFlamesEnabled = "nada.vfx.orbitals.flames.enabled";
        internal const string OrbitalsFlamesCount = "nada.vfx.orbitals.flames.count";
        internal const string OrbitalsFlamesHueShift = "nada.vfx.orbitals.flames.hue";
        internal const string OrbitalsFlamesEnergy = "nada.vfx.orbitals.flames.energy";
        internal const string OrbitalsFlamesSpacing = "nada.vfx.orbitals.flames.spacing";
        internal const string OrbitalsFlamesRadius = "nada.vfx.orbitals.flames.radius";

        internal const string OrbitalsEmbersEnabled = "nada.vfx.orbitals.embers.enabled";
        internal const string OrbitalsEmbersCount = "nada.vfx.orbitals.embers.count";
        internal const string OrbitalsEmbersHueShift = "nada.vfx.orbitals.embers.hue";
        internal const string OrbitalsEmbersEnergy = "nada.vfx.orbitals.embers.energy";
        internal const string OrbitalsEmbersSpacing = "nada.vfx.orbitals.embers.spacing";
        internal const string OrbitalsEmbersRadius = "nada.vfx.orbitals.embers.radius";
    }
}